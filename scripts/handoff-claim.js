...
async function main() {
  ...
  // Accepted statuses: setup_in_progress (legacy git clone), packaged, ready_to_claim (cos-handoff.sh)
  const validStatuses = ['setup_in_progress', 'setup_complete', 'packaged', 'ready_to_claim'];
  if (delegated.handoff_status && !validStatuses.includes(delegated.handoff_status)) {
    console.error(`⚠️  예상하지 못한 상태입니다: ${delegated.handoff_status}`);
  }

  const isDemoMode = delegated.demo_mode === true;
  ...
  // ─── Demo Transfer Path ───
  if (isDemoMode) {
    console.log('📺 Demo 모드 인수 — Operator → 고객 계정 이전\n');

    // 1. setup-progress 리셋 (cf-login부터 다시 → 고객 계정에 D1/R2/Pages 생성)
    const progressPath = path.join(PROJECT_ROOT, '.agent', 'setup-progress.json');
    if (fs.existsSync(progressPath)) {
      const progress = fs.readJsonSync(progressPath);
      // cf-login, device-register를 pending으로 리셋
      for (const step of progress.steps) {
        if (['cf-login', 'device-register'].includes(step.id)) {
          step.status = 'pending';
          step.error = undefined;
        }
      }
      // demoMode 컨텍스트 제거, demo_transfer 표시 (이제 정상 모드)
      delete progress.context.demoMode;
      delete progress.context.cfTokenMode;
      delete progress.context.cfAccountId;
      delete progress.context.dbId;
      // demo_transfer: cf-login에서 새 리소스 생성, device-register에서 새 등록
      progress.context.demoTransfer = true;
      fs.writeJsonSync(progressPath, progress, { spaces: 2 });
      console.log('   ✅ setup-progress 리셋 (cf-login, device-register → pending)');
    }

    // 2. wrangler.toml의 CF 리소스 ID를 placeholder로 리셋
    //    클라이언트가 cf-login 단계에서 자기 계정에 새로 생성
    const wranglerPath = path.join(PROJECT_ROOT, 'wrangler.toml');
    if (fs.existsSync(wranglerPath)) {
      let toml = fs.readFileSync(wranglerPath, 'utf8');
      // D1 database_id
      toml = toml.replace(/database_id\s*=\s*"[^"]+"/, 'database_id = "your-database-id-here"');
      // Pages project name (demo slug → placeholder for client to set)
      const currentName = toml.match(/^name\s*=\s*"([^"]+)"/m)?.[1];
      if (currentName && currentName.startsWith('cos-')) {
        toml = toml.replace(/^name\s*=\s*"[^"]+"/m, 'name = "my-clinic"');
        console.log(`   ✅ wrangler.toml name: "${currentName}" → "my-clinic" (클라이언트가 cf-login에서 재설정)`);
      }
      // R2 bucket name
      const currentBucket = toml.match(/bucket_name\s*=\s*"([^"]+)"/)?.[1];
      if (currentBucket && currentBucket.startsWith('cos-')) {
        toml = toml.replace(/bucket_name\s*=\s*"[^"]+"/, 'bucket_name = "my-clinic-uploads"');
        console.log(`   ✅ wrangler.toml bucket: "${currentBucket}" → "my-clinic-uploads"`);
      }
      fs.writeFileSync(wranglerPath, toml);
      console.log('   ✅ wrangler.toml database_id → placeholder 리셋');
    }

    // 3. delegated-setup.json 상태 업데이트
    delegated.handoff_status = 'claimed';
    delegated.claimed_at = new Date().toISOString();
    delegated.demo_transfer = true;
    fs.writeJsonSync(delegatedPath, delegated, { spaces: 2 });
    console.log('   ✅ delegated-setup.json → claimed (demo transfer)');

    // 4. softgate-state.json 리셋
    ...
    console.log('\n✅ Demo 인수 준비 완료!');
    console.log('   다음 단계:');
    console.log('   1. npx wrangler login  (이미 완료)');
    console.log('   2. npm run setup:step -- --step=cf-login  (D1/R2/Pages 생성)');
    console.log('   3. npm run setup:step -- --step=device-register');
    console.log('   4. npm run build && npm run deploy');
    console.log('   5. AI 코딩 에이전트로 온보딩 진행\n');
    process.exit(0);
  }
