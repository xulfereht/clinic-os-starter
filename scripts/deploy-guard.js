...
const IS_AGENT = !!process.env.CLAUDE_CODE || !!process.env.CURSOR_SESSION || !!process.env.CLINE_TASK;
const IS_NON_INTERACTIVE = CLI_ARGS.has('--non-interactive') || process.env.CI === 'true' || IS_AGENT;
const AUTO_YES = CLI_ARGS.has('--yes') || CLI_ARGS.has('-y') || IS_AGENT;
const SKIP_SECRETS = CLI_ARGS.has('--skip-secrets') || IS_NON_INTERACTIVE || AUTO_YES;
...
const MAX_ROUTES = 100; // Cloudflare Pages _routes.json limit

const OPTIONAL_SECRETS = ['ALIGO_API_KEY', 'ALIGO_USER_ID', 'ALIGO_SENDER', 'GOOGLE_AUTH_SECRET'];

async function ask(question, defaultValue = '', timeoutMs = 5000) {
    ...
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
            if (!answered) {
                answered = true;
                rl.close();
                if (IS_NON_INTERACTIVE || process.stdin.isTTY === false) {
                    console.log(` (timeout, using default: "${defaultValue}")`);
                }
...
// Resolve wrangler binary: prefer local node_modules, fallback to npx
const WRANGLER_BIN = (() => {
    const local = path.join(PROJECT_ROOT, 'node_modules', '.bin', 'wrangler');
    if (fs.existsSync(local)) return local;
    // Starter kit: wrangler may be in core/node_modules
    const coreLocal = path.join(PROJECT_ROOT, 'core', 'node_modules', '.bin', 'wrangler');
    if (fs.existsSync(coreLocal)) return coreLocal;
    return 'npx wrangler'; // fallback
})();

async function runCommand(cmd, silent = false, timeoutMs = 120000) {
    // Replace 'npx wrangler' with resolved binary to avoid npm path issues
    const resolvedCmd = cmd.replace(/^npx wrangler\b/, WRANGLER_BIN);
    if (!silent) console.log(`   Running: ${resolvedCmd}`);
    try {
        const { stdout, stderr } = await execAsync(resolvedCmd, { cwd: PROJECT_ROOT, timeout: timeoutMs });
        return { success: true, stdout, stderr };
    } catch (error) {
        if (error.killed) {
            error.message = `타임아웃 (${Math.round(timeoutMs/1000)}초): ${resolvedCmd}`;
        }
...
async function deployGuard() {
    ...
    // 1.5 A2: Check build manifest (build-to-deploy handoff)
    // Starter kit: build output is in core/dist/, not dist/
    const isStarterKit = fs.existsSync(path.join(PROJECT_ROOT, 'core', 'package.json'));
    const earlyDistDir = isStarterKit ? 'core/dist' : 'dist';
    const buildManifestPath = path.join(PROJECT_ROOT, earlyDistDir, '.build-manifest.json');
    if (fs.existsSync(buildManifestPath)) {
        try {
            const buildManifest = fs.readJsonSync(buildManifestPath);
            console.log(`📋 Build manifest found (built: ${buildManifest.buildAt})`);
            if (!buildManifest.routesWithinLimit) {
                console.error(`   ❌ _routes.json has ${buildManifest.routeCount} rules (exceeds ${MAX_ROUTES} limit). Cloudflare will reject.`);
                blockers++;
            }
            // Verify output dir matches what we expect
            const expectedDistDir = path.join(PROJECT_ROOT, buildManifest.outputDirRelative || 'dist');
            if (!fs.existsSync(expectedDistDir)) {
                console.error(`   ❌ Build output dir ${buildManifest.outputDirRelative} doesn't exist. Re-run: npm run build`);
                blockers++;
            } else {
                console.log(`   ✅ Build output verified: ${buildManifest.outputDirRelative} (${buildManifest.routeCount} routes)\n`);
            }
        } catch (e) {
            console.warn(`   ⚠️  Build manifest unreadable: ${e.message}\n`);
        }
    } else {
        console.warn("⚠️  No .build-manifest.json found. Run 'npm run build' before deploying.\n");
        warnings++;
    }
        ...
        if (fs.existsSync(playwrightConfig)) {
            try {
                ...
                if (!e2eResult.success) {
                    ...
                    if (IS_NON_INTERACTIVE || AUTO_YES) {
                        console.log('   ℹ️  비대화형/자동 모드: E2E 실패는 경고로 처리합니다.');
                        warnings++;
                    } else {
