...
async function setupCoreViaGit(hqUrl, deviceToken) {
    ...
    if (!fs.existsSync(path.join(corePath, '.git'))) {
        ...
        const updateCmd = `git fetch --tags && (git checkout v${version} || git checkout ${version})`;
...
`;
    ...
    // Sanitize clinic name for DB/Bucket
    ...
    if (fs.existsSync(migrationPath)) {
        ...
        if (fs.existsSync(localD1StatePath)) {
            ...
            try {
                ...
                console.log("   ⚠️  상태 초기화 중 오류 (파일 장금 등):", e.message);
            }
        }
