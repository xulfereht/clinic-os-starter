...
// Agent mode detection for child process propagation
const CLI_ARGS = new Set(process.argv.slice(2));
const IS_AUTO = CLI_ARGS.has('--auto') || CLI_ARGS.has('--yes') || CLI_ARGS.has('-y') ||
                !!process.env.CLAUDE_CODE || !!process.env.CI || process.env.NODE_ENV === 'test';
const AUTO_ENV = IS_AUTO ? { ...process.env, CLAUDE_CODE: process.env.CLAUDE_CODE || '1', CI: process.env.CI || 'true' } : process.env;
...
async function runCfLogin(progress) {
  ...
  // 5-B. Pages D1/R2 바인딩 자동 설정 (Cloudflare API)
  if (pagesProjectName && cfAccountId && dbId) {
    console.log('   🔗 Pages D1/R2 바인딩 설정 중...');
    try {
      const bindingPayload = {
        deployment_configs: {
          production: {
            ...
            ...(bucketName ? { r2_buckets: { BUCKET: { name: bucketName } } } : {})
          }
        }
      };

      // wrangler OAuth 토큰 또는 API 토큰으로 인증
      let authHeader;
      if (hasToken) {
        authHeader = `Bearer ${process.env.CLOUDFLARE_API_TOKEN}`;
      } else {
        // wrangler OAuth 토큰 읽기
        ...
        if (fs.existsSync(wranglerConfigPath)) {
          ...
          const oauthMatch = wranglerConfig.match(/oauth_token\s*=\s*"([^"]+)"/);
          if (oauthMatch) authHeader = `Bearer ${oauthMatch[1]}`;
        }
      }
      ...
      if (authHeader) {
        ...
        if (result.success) {
          console.log('   ✅ Pages D1/R2 바인딩 설정 완료');
        } else {
          console.log(`   ⚠️  Pages 바인딩 설정 실패: ${JSON.stringify(result.errors)}`);
          console.log('      배포 후 Cloudflare 대시보드에서 수동 설정이 필요할 수 있습니다.');
        }
      } else {
        console.log('   ⚠️  인증 토큰을 찾을 수 없어 Pages 바인딩 설정을 건너뜁니다.');
      }
    } catch (e) {
      console.log(`   ⚠️  Pages 바인딩 설정 오류: ${e.message}`);
    }
  }
...
async function runDeviceRegister(progress) {
  ...
  // Demo Transfer: 핸드오프 후 클라이언트가 재실행
  // license_key가 없을 수 있음 — 수동 등록 안내
  if (progress.context.demoTransfer && !progress.context.licenseKey) {
    console.log('   📦 Demo Transfer — 라이선스 키가 없습니다.');
    console.log('   clinic.json에 license_key를 추가하거나,');
    console.log('   HQ(https://clinic-os-hq.pages.dev)에서 라이선스를 신청하세요.');
    console.log('   라이선스 발급 후: npm run setup:step -- --step=device-register\n');
    progress.context.hqUrl = DEFAULT_HQ_URL;
    // 실패가 아닌 스킵 — 나중에 다시 실행 가능
    return { success: true };
  }
...
async function runNpmInstallSafe(cwd, label) {
  ...
  await execAsync(installCmd, {
    cwd,
    env: { ...AUTO_ENV, NODE_OPTIONS: '--max-old-space-size=512' },
    timeout: 300000 // 5분
  });
...
async function runDbMigrate(progress) {
  ...
  if (fs.existsSync(migratePath)) {
    ...
    // 리모트 D1에도 마이그레이션 적용 (위임 셋업, 데모 모드 등)
    const shouldApplyRemote = isRemoteMode || progress.context.demoMode;
...
async function runDbSeed(progress, seedType) {
  ...
  try {
    ...
    // 리모트에도 시드 적용 (위임 셋업, 데모 모드 등)
    if (isRemoteMode || progress.context.demoMode) {
...
async function main() {
  ...
  // 한 번 설정되면 progress.context.demoMode로 지속됨 (플래그 누락 방어)
  const progressForModeCheck = fs.existsSync(PROGRESS_PATH) ? fs.readJsonSync(PROGRESS_PATH) : {};
  const isDemoMode = args.includes('--demo-mode')
    || process.env.CLINIC_DEMO_MODE === 'true'
    || progressForModeCheck?.context?.demoMode === true;
