...
/**
 * Auto-install dependencies if node_modules missing or dependencies actually changed
 * (package.json의 dependencies 해시를 비교하여 mtime 오탐 방지)
 */
function ensureDependencies() {
    ...
    // dependencies + devDependencies 해시로 실제 변경 감지
    try {
        ...
        const currentHash = createHash('md5').update(depsStr).digest('hex');

        let savedHash = '';
        if (fs.existsSync(hashFile)) {
            savedHash = fs.readFileSync(hashFile, 'utf8').trim();
        }
