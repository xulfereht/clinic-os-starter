...
Page-level hero header with optional background image.
...
| `theme` | `"light" \| "dark" \| "glass"` | | Visual theme (default: `"light"`) |
| `skin` | `string` | | Skin ID (default: `"clinicLight"`) |

### HeroBridge

Combined hero + bridge section with large decorative symbol. Falls back to default text if no data provided.

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `title` | `string` | | Main heading (supports `<br/>`) |
| `description` | `string` | | Supporting paragraph |
| `data` | `{ title, description, [key]: any }` | | Alternative data source (overrides direct props) |

### DoctorList

Auto-populated from `staff` table — renders all active doctors. No manual data needed.

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `data` | `object` | | Reserved for future use |

> **Note**: Fetches from `staff` table where `type='doctor'` and `is_active=1`. Supports translations via `staff_translations`.

### DiagnosisList

Auto-populated from `self_diagnosis_templates` table — renders active self-diagnosis tools.

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `data.emptyText` | `string` | | Empty state message |
| `data.startText` | `string` | | CTA button text |
| `data.timeUnit` | `string` | | Time unit label (default: "분") |
| `data.approxStr` | `string` | | Approximation prefix (default: "약") |
| `data.otherCategory` | `string` | | Fallback category name (default: "기타") |

> **Note**: Fetches active templates from DB. Supports translations via `self_diagnosis_template_translations`.

### ProgramList

Auto-populated from `programs` table — renders visible programs as card grid.

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `data.viewMoreText` | `string` | | Card CTA text (default: "자세히 보기") |
| `data.defaultDesc` | `string` | | Fallback description for programs without one |

> **Note**: Fetches from `programs` where `is_visible=1`. Uses skin page template for card styling.

### TelemedicineHero

Landing section for telemedicine/remote consultation programs.

| Prop | Type | Required | Description |
|------|------|----------|-------------|
| `data.tag` | `string` | | Top badge text (default: "비대면 진료") |
| `data.title` | `string` | | Main heading (supports `<br/>`) |
| `data.description` | `string` | | Supporting description |
| `data.principles` | `{ title, description }` | | Care philosophy section |
| `data.cases` | `{ title, items: string[] }` | | Recommended cases list |
