/**
 * Clinic-OS Core Pull (Local Git Architecture v1.2)
 *
...
async function mergePackageJson(targetTag) {
    const localPkgPath = path.join(PROJECT_ROOT, 'package.json');
...
async function runNewMigrations(migrationFiles) {
    ...
    for (const migFile of migrationFiles) {
        ...
        const filePath = path.join(PROJECT_ROOT, migFile);
...
async function corePull(targetVersion = 'latest') {
    console.log('🚢 Clinic-OS Core Pull v4.1 (Local Git Architecture v1.2)\n');
    ...
    if (engineQueue.length > 0) {
        ...
        for (const { status, path: filePath } of engineQueue) {
            if (status === 'D') {
                const fullPath = path.join(PROJECT_ROOT, filePath);
                if (fs.existsSync(fullPath)) {
                    fs.removeSync(fullPath);
