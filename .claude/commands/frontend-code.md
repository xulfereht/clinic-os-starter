...
|------|---------|----------------|
| **Homepage custom** | "이 느낌으로 홈페이지 만들어줘" | `custom-homepage/pages/index.astro` |
| **Page override** | "의료진 페이지 다르게 만들고 싶어" | `src/pages/_local/{path}.astro` |
| **New page** | "랜딩페이지 하나 만들어줘" | `src/plugins/local/{id}/pages/` |
| **Custom skin** | "이 사이트 색감으로 스킨 만들어" | `src/skins/local/{name}/` |
| **Section add** | "홈페이지에 타임라인 섹션 추가" | `custom-homepage/pages/index.astro` |
| **Component** | "재사용 가능한 FAQ 아코디언" | `src/lib/local/{name}.astro` |
...
adaptation_notes: |
  Clinic-OS에서 구현할 때:
  - CSS 변수(--accent, --bg-body 등) 활용
  - SectionRenderer 사용 가능한 섹션은 SectionRenderer로
  - 커스텀 섹션은 인라인 HTML로 직접 작성
  - 이미지는 public/local/에 배치
```

**Multiple references**: 여러 URL이 주어지면 각각 분석 후 "best of" 조합.
...
1. **Tailwind 4 CSS variables** — Clinic-OS skin system과 호환:
   ```html
   <!-- ✅ skin-aware -->
...
2. **Mobile-first responsive** — 한국 한의원 환자 80%+ 모바일:
   ```html
...
- "여백 좀 더 줘" → increase py/px values
- "폰트 더 크게" → bump text-* scale
- "색 좀 바꿔" → adjust CSS variables in skin or inline
- "이 섹션 순서 바꿔" → move HTML blocks
- "새 섹션 추가해" → write new section HTML
- "레퍼런스 사이트의 이 부분처럼" → WebFetch specific element, adapt
...
| Skill | Relationship |
|-------|-------------|
| `/analyze-content` | style-card.yaml → 톤앤매너 참조 |
| `/discover-edge` | edge-profile.yaml → 카피 방향, USP |
| `/write-copy` | 카피 텍스트 → 데이터 객체에 삽입 |
| `/setup-homepage` | 프리셋 기반 데이터 수정 (이 스킬은 그 너머) |
| `/curate-images` | asset-metadata.json → 이미지 경로 참조 |
| `/setup-skin` | 기존 스킨 선택 (이 스킬은 새 스킨 생성 가능) |
| `/plan-content` | site-plan.yaml → 섹션 구성 참조 |
| `/review-compliance` | 완성된 카피 → 의료광고 심의 |
