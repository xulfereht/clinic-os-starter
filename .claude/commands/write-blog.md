...
- "블로그 글 써줘" — generate a new blog post
- Content pipeline step — after analyze-content + discover-edge
- Ongoing content production — "이번 주 블로그"
- Topic-driven — "다이어트 관련 글 써줘"
...
```bash
npx wrangler d1 execute DB --local --command \
  "INSERT INTO posts (slug, title, content, excerpt, type, category, status, created_at, updated_at)
   VALUES (
     '{slug}',
     '{title}',
     '{content_html}',
...
Blog posts are subject to medical advertising review. Run the same pattern check as /write-copy before saving to DB.

**Blog-specific cautions:**
- Before/after treatment photos prohibited
- Patient testimonials may be interpreted as treatment guarantees
- "OO specialist" without board certification prohibited
- "Complete cure", "guaranteed recovery" expressions prohibited
- When citing research, use "reported" not "proven"

> Reference: Medical Act Article 56, MOHW Medical Advertising Guidelines (2nd ed.)

## Onboarding State Sync

After blog post is saved to DB, mark the onboarding feature as done.
This runs regardless of entry point (demo, delegated, onboarding, standalone).

```bash
npm run onboarding:done -- --feature=blog-management --note="write-blog 완료"
```
