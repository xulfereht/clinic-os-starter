...
# Table list
npx wrangler d1 execute DB_NAME --local --command ".tables" 2>&1
...
🔧 자동 수리 가능한 항목:
  ...
  [2] DB 스키마 자동 복구 (npm run doctor)
  [3] 리모트 마이그레이션 적용
  [4] _local/ 파일 import 경로 수정
...
```bash
./scripts/cos-ask "에러 메시지 + 시도한 해결책 요약"
```
...
문제: npm run build 실패 — Cannot find module '@astrojs/cloudflare'
원인: node_modules 손상
해결: npm install 재실행 → 빌드 성공
...
|------|-----------|
| "빌드 안 돼요" | `rm -rf node_modules && npm install && npm run build` |
| "500 에러" | `npx wrangler d1 migrations apply {db} --remote` |
| "이미지 깨져요" | `ls public/local/` → check paths → fix |
| "관리자 접속 안 돼요" | `grep ADMIN_PASSWORD wrangler.toml` → verify |
| "업데이트 후 깨졌어요" | `npm run health:fix` |
