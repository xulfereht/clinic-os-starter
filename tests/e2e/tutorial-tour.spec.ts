...
test.describe('Interactive Tutorial Tour System', () => {
    ...
    // ── AC-5: Tour UI renders in sidebar ──
    test('AC-5: Tour progress visible in admin sidebar', async ({ page }) => {
        // beforeEach already logged in and landed on /admin
        await page.waitForLoadState('networkidle');

        // Wait for React hydration of TourProgress component
        const sidebar = page.locator('#sidebar');
        // Should find tour-related content (progress bar or toggle button)
        const tourText = sidebar.locator('text=/튜토리얼|투어|풀코스|시작하기|이어하기|끄기/');
        await expect(tourText.first()).toBeVisible({ timeout: 15000 });
    });
