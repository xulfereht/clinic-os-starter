---
description: Full content bootstrap pipeline — from data collection through image
  generation to site launch. Agent-executable for both onboarding and delegated flows.
category: content
---
...
Phase 1.5: Reference Collection (optional, recommended)
  └─ Competitor/design/existing site analysis → references.yaml
...
## 기획 산출물 경로 (Phase 간 핸드오프)

| 산출물 | 경로 | Phase | 소비 |
|--------|------|-------|------|
| 클리닉 프로파일 | `.agent/clinic-profile.json` | 1 (수집) | Phase 2, 5 |
| 스타일 카드 | `.agent/style-card.yaml` | 2 (분석) | Phase 3, 6 |
| 엣지 프로파일 | `.agent/edge-profile.yaml` | 3 (강점) | Phase 4, 6 |
| 레퍼런스 분석 | `.agent/references.yaml` | 1.5 (수집) | Phase 2, 5, 6 |
| 파이프라인 컨텍스트 | `.agent/pipeline-context.yaml` | 2+ (누적) | 각 Phase가 읽고 업데이트 |
| 사이트 플랜 | `.agent/site-plan.yaml` | 3 (기획) | Phase 5, 6 |
| 에셋 이미지 | `public/local/homepage/` | 6b (소싱) | Phase 6c |
| 에셋 메타데이터 | `public/local/homepage/asset-metadata.json` | 6d | 에이전트 참조 |

> 모든 `.agent/*.yaml`은 local (core:pull 보호). 후속 스킬은 이 파일들을 읽어 자동 채움.
...
Ask if the client has reference URLs to provide:
- 기존 홈페이지 URL (이전 중인 사이트)
- 경쟁 한의원 사이트 (벤치마크 대상, 1-3개)
- 디자인 참고 사이트

If provided → run `/collect-references` → `.agent/references.yaml` generated.
If not provided → skip (downstream skills will work without references, with reduced context).

**Sufficiency checkpoint:**
After Phase 1 + 1.5, assess data quality:
- Blog posts with content: {N} — thin (<5) / adequate (5-30) / rich (30+)
- Place data: missing / partial / ok
- References: none / available
- Overall: ready / needs-supplement / insufficient

If `needs-supplement`: proceed but note data gaps at each downstream step.
If `insufficient`: warn the user and suggest providing more data before continuing.
...
```bash
# 프리셋 복사 → local/ 경로에 활성화 (core:pull 보호)
...
cp core/src/plugins/custom-homepage/presets/editorial.astro \
   core/src/plugins/local/custom-homepage/pages/index.astro
```

> plugin-loader는 `local/custom-homepage/`를 자동으로 우선 사용합니다.
> 코어의 presets/는 읽기 전용 템플릿, 커스터마이징은 local/ 복사본에서.

프리셋은 시작점입니다. 복사 후 콘텐츠 데이터(hero, credentials, papers 등)를 자유롭게 수정하세요.
빈 배열/빈 문자열로 설정하면 해당 섹션이 자동으로 숨겨집니다.
이미지는 반드시 `public/local/homepage/`에 배치하세요 (URL: `/local/homepage/파일명.jpg`).
...
5. core/public/local/homepage/optimized/에 배치 (local/ 경로 = core:pull 보호)
```
...
정적 에셋:     core/public/local/homepage/optimized/  ← local/ 경로 필수 (보호됨)
```
...
[ ] Phase 1.5: References collected (or explicitly skipped)
[ ] Sufficiency checkpoint passed
[ ] Phase 2: Style card written, programs planned, image matrix ready
[ ] site-plan.yaml readiness block reviewed
...
## alignment-report.yaml Schema

`/align-programs` 스킬 출력물. 프로그램/콘텐츠가 tacit-product.yaml과 정렬되는지 검증 결과.
...
**사용 시점:**
- 월 1회 주기적 검증
- tacit 업데이트 후 기존 프로그램 검증
- 신규 콘텐츠 작성 전 사전 검증 (`--preview`)

---

## Pipeline Context Schema

`.agent/pipeline-context.yaml`는 파이프라인 단계 간 누적되는 상태 파일입니다.

### tacit_product 섹션 (v1.0)

`/extract-tacit-product` 스킬 완료 후 업데이트됩니다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| status | enum | pending (미시작), in_progress (진행중), complete (완료) |
| confidence | float | LLM 평가 답변 일관성 (0.75+ 권장) |
| axes_complete | string[] | 완료된 7축 목록 |
| axes_gaps | string[] | 정보 부족 축 목록 (follow-up 필요) |

**다운스트림 사용:**
- `/write-copy` → positioning.claim, communication.hook
- `/setup-homepage` → positioning, communication 섹션
- `/setup-programs` → personalization.axes, definition
